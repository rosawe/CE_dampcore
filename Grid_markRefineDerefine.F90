...
!! added this to prevent forming too many blocks
     gr_enforceMaxRefinement = .TRUE.
     call Logfile_stamp(gr_maxRefine,'[Grid_markRefineDerefine] exceeded maxblocks: set gr_maxRefine')
  end if
...
!!$  force_consistency = .TRUE.
  ...
  if(gr_lrefineMaxRedDoByLogR) then
       ...
       !sam
       call Logfile_stamp(gr_maxRefine,'[Grid_markRefineDerefine] refining by radius')
  end if
  ...
  if (dr_simTime .eq. 0.d0) then
      ...
      if (refinement_type .eq. 'rel_to_max') then
          ...
          call Logfile_stamp(max_dens,'[Grid_markRefineDerefine] max_dens is ')
