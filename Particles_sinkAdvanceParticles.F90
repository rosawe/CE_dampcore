...
subroutine Particles_sinkAdvanceParticles(dr_dt)
  ...
  use Simulation_data, only: sim_fixedPartTag, sim_tRelax, sim_comAccel, &
                             sim_mpoleVX, sim_mpoleVY, sim_mpoleVZ, sim_vyi, sim_startY
...
!!      v(t+dt)    = v(t)       + dt * 1/2* [ a(x(t),t) + a(x(t+dt),t) ]
  ...
  if (integrator .eq. 3) then   ! Leapfrog Cosmo
     ...
     sOld = a0
     if (dr_simTime .LE. 5d-10) then
        write(logstr, fmt='(A30, 6ES15.8)') 'Pt. mass position (1):', particles_global(POSY_PART_PROP, 2)
        call Logfile_stampMessage(logstr)
     
        write(logstr, fmt='(A30, 6ES15.8)') '(local) Pt. mass position (1):', particles_local(POSY_PART_PROP, 2)
        call Logfile_stampMessage(logstr)
     end if
    ...
    ! JFG - Subtract velocities from Driver_sourceTerms from particle motion.
     if (sink_AdvanceSerialComputation) then
         ...
         do i = 1, np
                ...
                !moved this up because particle trajectory affected
     ...
     if (dr_simTime .LE. 5d-10) then
        write(logstr, fmt='(A30, 6ES15.8)') 'Pt. mass velocity 3:', particles_global(VELX_PART_PROP, 2)
         call Logfile_stampMessage(logstr)

         write(logstr, fmt='(A30, 6ES15.8)') '(local) Pt. mass velocity 3', particles_local(VELX_PART_PROP, 2)
         call Logfile_stampMessage(logstr)
     endif
     ...
     if (dr_simTime .LE. 5d-10) then
        write(logstr, fmt='(A30, 6ES15.8)') 'Pt. mass position (2):', particles_global(POSY_PART_PROP, i)
        call Logfile_stampMessage(logstr)
     
        write(logstr, fmt='(A30, 6ES15.8)') '(local) Pt. mass position (2):', particles_local(POSY_PART_PROP, i)
        call Logfile_stampMessage(logstr)
     end if
         end do
     ...
     if (dr_simTime .LE. 5d-10) then
        write(logstr, fmt='(A30, 6ES15.8)') 'Pt. mass position (3):', particles_global(POSY_PART_PROP, i)
        call Logfile_stampMessage(logstr)
     
        write(logstr, fmt='(A30, 6ES15.8)') '(local) Pt. mass position (3):', particles_local(POSY_PART_PROP, i)
        call Logfile_stampMessage(logstr)
     end if
         end do
...
!  !ic trajectory: sam
!  if (dr_simTime .le. sim_tRelax) then
!        call pt_sinkGatherGlobal()
!         do i = 1, localnpf
!             if (idnint(particles_global(TAG_PART_PROP,i)) .ne.sim_fixedPartTag) then
!                 call ic_trajectory(dr_simTime,particles_global(POSY_PART_PROP,i),sim_startY,sim_vyi)
!                 print *, '[Particles_sinkAdvance]', particles_global(POSY_PART_PROP,i) 
!             endif
!         enddo
!  endif
