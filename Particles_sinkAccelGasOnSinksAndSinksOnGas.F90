...
subroutine Particles_sinkAccelGasOnSinksAndSinksOnGas(accelProps,accelVars)
  ...
  !JFG
  use Simulation_data, ONLY : sim_gravityType, sim_tRelax
  !End JFG
  !sam
...
!!$     call Driver_abortFlash('Particles_sinkAccelGasOnSinksAndSinksOnGas: ERROR in index of particle property.')
  ...
  do lb = 1, blockCount
     ...
     blockID = blockList(lb)
           ...
           do j = blkLimits(LOW,JAXIS), blkLimits(HIGH,JAXIS)
              do i = blkLimits(LOW,IAXIS), blkLimits(HIGH,IAXIS)
                 ...
                 do p = 1, localnpf
                       ...
                       !sam: for turning on gravity during relaxation
                       call damping(dr_simTime,sim_tRelax,damp_const)
                       ! sinks on gas accelerations
                       paccx = paccx - ax * pmass
                       paccy = paccy - ay * pmass
                       paccz = paccz - az * pmass
                       !sam: for gradually increasing gravity during relaxation
                       paccx = damp_const * paccx
                       paccy = damp_const * paccy
                       paccz = damp_const * paccz
        ...
        !if (lb .eq. 1) print *, 'Particles_sinkAccelGasOnSinksAndSinksOnGas:', &
        !        solnData(accxVar,blkLimits(LOW,IAXIS),blkLimits(LOW,JAXIS), &
        !        blkLimits(LOW,KAXIS))         
